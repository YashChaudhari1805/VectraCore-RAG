...
- **Any-Match Rate**  — correct bot appears anywhere in matched results.
- **Mean Similarity** — average cosine score of the expected bot across all posts.
...
from core.router import build_index, get_all_scores

_TEST_POSTS: list[dict[str, str]] = [
...
def run_eval(threshold: float = 0.18) -> dict:
    """
    Run the full router evaluation suite and print a human-readable report.

    Args:
        threshold: Cosine similarity cutoff used for Any-Match Rate calculation.

    Returns:
        A dict summarising evaluation results with keys:
        ``top1_accuracy``, ``any_match_rate``, ``mean_similarity``,
    ...
    per_bot: dict[str, dict[str, int]] = {
        bot: {"total": 0, "top1": 0, "any": 0} for bot in _KNOWN_BOT_IDS
    }

    print(f"\n{'#':<4} {'Expected':<24} {'Got #1':<24} {'Score':>7}  {'✓'}")
    print("─" * 65)

    for idx, test in enumerate(_TEST_POSTS, start=1):
        post = test["post"]
        expected = test["expected_top_bot"]
        scores = get_all_scores(post)

        top1_bot_id: str = scores[0]["bot_id"] if scores else "none"
        expected_score: float = next(
            (s["similarity"] for s in scores if s["bot_id"] == expected), 0.0
        )
        correct_scores.append(expected_score)

        matched_ids: list[str] = [
            s["bot_id"] for s in scores if s["similarity"] >= threshold
        ]

        is_top1 = top1_bot_id == expected
        ...
        if is_top1:
            per_bot[expected]["top1"] += 1
        if is_any:
            per_bot[expected]["any"] += 1

        tick = "✅" if is_top1 else ("⚠️ " if is_any else "❌")
        short_expected = expected.replace("Bot_", "").replace("_", " ")
        short_top1 = top1_bot_id.replace("Bot_", "").replace("_", " ")
        print(f"{idx:<4} {short_expected:<24} {short_top1:<24} {expected_score:>6.4f}  {tick}")
    ...
    for bot_id, stats in per_bot.items():
        ...
        print(f"  {name:<20} Top-1: {stats['top1']}/{t}  Any-match: {stats['any']}/{t}")
